<template>
                    ...
                    <VRow>
                        <VCol cols="12" md="2">
                            <VDialog v-model="ajouter" persistent class="v-dialog-md">
                                ...
                                <!-- Dialog Content -->
                                <VCard title="Nouvelle Agape ?">
                                    <VForm ref="newAgape" @submit.prevent="newAgape">
                                        <VCardText>
                                            <VRow>
                                                <VCol md="8" cols="12">
    ...
    <VCard>
        <VCardText>
            <VRow>
                <v-data-table :headers="headers" :items="Agapes" item-value="name" :search="search"
                    :items-per-page="4">
                                ...
                                <v-list>
                                    <v-list-item>
                                        ...
                                        <v-list-item-title :class="item.raw.status != 'actif' ? 'd-none' : ''">
                                            <VBtn type="submit" color="" size="small" @click="desableAgape(item.raw)">
                                        ...
                                        <v-list-item-title :class="item.raw.status != 'inactif' ? 'd-none' : ''">
                                            <VBtn type="submit" color="" size="small" @click="enableAgape(item.raw)">
                                        ...
                                        <v-list-item-title>
                                            <VBtn type="submit" color="" size="small" @click="deleteAgape(item.raw)">
    ...
    <VDialog v-model="modifier" persistent class="v-dialog-md">
        <!-- Dialog Content -->
        <VCard title="Modification d'une Agape ">
            <VCardText>
                <VForm ref="updateAgape" @submit.prevent="updateAgape">
    ...
    <VDialog v-model="consulter" persistent class="v-dialog-md">
        <VCard title="Informations d'une Agape">
            <VCardText>
                <VForm ref="viewAgape" @submit.prevent="viewAgape">
                    <VRow class="text-center mb-1">
                        <VCol cols="12" md="8">
                            <VTextField label="Noms & Prénoms" density="compact" readonly
                                prepend-inner-icon="mdi-account-outline" v-model="account.nom_complet" />
...
export default {
    ...
    methods: {
        async newAgape() {
            try {
                if (this.$refs.newAgape.validate()) {
                    let utilisateur = this.account.utilisateur_id.split('-)')
                    Swal.fire({
                        title: 'Êtes vous sûres ?',
                        html: "Souhaitez vous effectuer le versement de <b style='color:red'>" + parseInt(data.montant) + " Fcfa</b> pour le compte de l'Agape de l'utilisateur <b>" + utilisateur[1].toUpperCase() + " </b>  ceci pour un montant de  ?",
                        showDenyButton: true,
        ...
        async updateAgape() {
            try {
                if (this.$refs.updateAgape.validate()) {
                    let utilisateur = this.account.utilisateur_id.split('-)')
                    Swal.fire({
                        title: 'Êtes vous sûres ?',
                        html: "Souhaitez vous modifier le versement de <b style='color:red'>" + parseInt(data.montant) + " Fcfa</b> pour le compte de l'Agape de l'utilisateur <b>" + utilisateur[1].toUpperCase() + " </b> ?",
                        showDenyButton: true,
                        showCancelButton: false,
                        confirmButtonText: "Je le souhaite",
                        denyButtonText: `Annuler`,
        ...
        async modification(data) {
            try {
                axios.get('check_Agape/' + data.id).then((response) => {
        ...
        async getAgape() {
            try {
                axios.get('Agape').then((response) => {
                    this.Agapes = response.data;
                }).catch((error) => {
                    console.log(error)
                });
            }
        ...
        async enableAgape(data) {
            console.log(data)
            Swal.fire({
                title: 'Êtes vous sûres ?',
                html: "Souhaitez vous désactivé l'Agape de <b>" + data.nom_complet.toUpperCase() + " </b>  ceci pour un montant de <b style='color:red'>" + parseInt(data.montant) + " Fcfa</b> ?",
                showDenyButton: true,
                showCancelButton: false,
                confirmButtonText: "Je le souhaite",
                denyButtonText: `Annuler`,
                ...
                if (result.isConfirmed) {
                    axios.post("on_Agape/" + data.id).then((response) => {
        ...
        async desableAgape(data) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.post("off_Agape/" + data.id).then((response) => {
                        if (response.data.status === 409) {
                            this.toast.error(response.data.alert, { position: 'top-right', duration: 7500 });
                        } else if (response.data.status === 200) {
                            this.getAgape();
        ...
        async deleteAgape(data) {
            Swal.fire({
                title: 'Êtes vous sûres ?',
                html: "Souhaitez vous supprimé l'Agape de <b>" + data.nom_complet.toUpperCase() + " </b> ceci pour un montant de <b style='color:red'>" + parseInt(data.montant) + " Fcfa</b> ?",
                showDenyButton: true,
                showCancelButton: false,
                confirmButtonText: "Je le souhaite",
                denyButtonText: `Annuler`,
                ...
                if (result.isConfirmed) {
                    axios.post("delete_Agape/" + data.id).then((response) => {
                        if (response.data.status === 409) {
                            this.toast.error(response.data.alert, { position: 'top-right', duration: 7500 });
                        } else if (response.data.status === 200) {
                            this.getAgape();
